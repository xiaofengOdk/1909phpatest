...
class Test extends Controller
{
    ...
    public function return_url(){
         $arr=$_GET;
          $order_info=  Order_info::where("order_sn",$_GET['out_trade_no'])->first();
         $goods_id= Order_goods::where("order_id",$order_info['order_id'])->get();
         Order_info::where("order_sn",$_GET['out_trade_no'])->update(['shipping_status'=>2,"pay_status"=>2,"payname"=>2,"add_time"=>time()]);
         foreach($goods_id as $k=>$v){
         $result=   Cary::where("goods_id",$v['goods_id'])->update(['is_del'=>3]);            
         }
         return redirect('/fina');
          $config=config("alipay");
        require_once app_path('libs/alipay/wappay/service/AlipayTradeService.php');
        require_once app_path('libs/alipay/wappay/buildermodel/AlipayTradeWapPayContentBuilder.php');
